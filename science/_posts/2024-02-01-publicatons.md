---
layout: plain
title: Publications
permalink: /science/publications/

sitemap: false
---

### Publications

- [*4MOST ChANGES: Catalog of high-redshift quasar candidates (4.5 < z < 7) selected with SED fitting*](https://doi.org/10.1051/0004-6361/202557171){:target="_blank"} — **Mkrtchyan, T.**, Mazzucchelli, C., Assef, R. J., et al. 2026, **A&A**, 710, A170.

- [*VAR-PZ: Constraining the Photometric Redshifts of Quasars using Variability*](https://doi.org/10.1051/0004-6361/202557223){:target="_blank"} — Satheesh-Sheeba, S., Assef, R. J., Anguita, T., et al. (incl. **Mkrtchyan, T.**) 2026, **A&A**, 708, A121.

- [*Discovery and characterization of 25 new quasars at 4.6 < z < 6.9 from wide-field multi-band surveys*](https://doi.org/10.1051/0004-6361/202554859){:target="_blank"} — Belladitta, S., Bañados, E., Xie, Z.-L., et al. (incl. **Mkrtchyan, T.**) 2025, **A&A**, 699, A335.

### Posters

- [*Characterizing High-Redshift Quasars (z>4.5) from 4MOST to LSST*]({{ '/assets/img/docs/poster_Mkrtchyan_2025_4MOST.pdf' | relative_url }}){:target="_blank"} — T. Mkrtchyan, presented at *SOCHIAS XX Annual Meeting*, Chile, 2025
- [*Searching for Lya halos around radio-loud quasars*]({{ '/assets/img/docs/poster_Mkrtchyan_2024_MUSE.pdf' | relative_url }}){:target="_blank"} — T. Mkrtchyan, presented at *Extreme galaxies in their extreme environments at extremely early epochs* conference, Iceland, 2024

---

For a complete and up-to-date list of publications, visit my [ADS Publication Profile](https://ui.adsabs.harvard.edu/search/filter_author_facet_hier_fq_author=AND&filter_author_facet_hier_fq_author=author_facet_hier%3A%220%2FMkrtchyan%2C%20T%22&filter_database_fq_database=AND&filter_database_fq_database=((database%3Aastronomy%20OR%20database%3Aphysics))&filter_database_fq_database=database%3A%22astronomy%22&fq=%7B!type%3Daqp%20v%3D%24fq_database%7D&fq=%7B!type%3Daqp%20v%3D%24fq_author%7D&fq_author=(author_facet_hier%3A%220%2FMkrtchyan%2C%20T%22)&fq_database=(((database%3Aastronomy%20OR%20database%3Aphysics))%20AND%20database%3A%22astronomy%22)&q=author%3A%22Mkrtchyan%2C%20Tatevik%22&sort=date%20desc%2C%20bibcode%20desc&p_=0){:target="_blank"}.


